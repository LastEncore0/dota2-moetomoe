...
function AIlelouch(hero, unit)
	...
	local ab3range = ab3:GetCastRange()
	local herolf = hero:GetHealthPercent()
	local funits = FindUnitsInRadius(hero:GetTeam(), hero:GetAbsOrigin(), nil, ab2range, DOTA_UNIT_TARGET_TEAM_FRIENDLY,
		DOTA_UNIT_TARGET_HERO, 0, 0, false)
	local bunits = FindUnitsInRadius(hero:GetTeam(), hero:GetAbsOrigin(), nil, ab3range, DOTA_UNIT_TARGET_TEAM_ENEMY,
		DOTA_UNIT_TARGET_BASIC, 0, 0, false)
...
function AIStart(hero)
	...
	Timers:CreateTimer(0.5, function()
		...
		if MTM_STOP_AI == nil then
			local units = FindUnitsInRadius(hero:GetTeam(), hero:GetAbsOrigin(), nil, 1600, DOTA_UNIT_TARGET_TEAM_ENEMY,
				DOTA_UNIT_TARGET_HERO, 0, 0, false)
			--print("ai unit")
...
function AIPush(hero)
	...
	--------------收集数据---------------
	local units = FindUnitsInRadius(hero:GetTeam(), pOrg, nil, 1200, DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_ALL,
	...
	elseif lifeper < 35 then
		...
		if hero.hpStreak[3] - hero.hpStreak[1] > 8 then
			...
			if #enemytowers > 0 then
				--周围有敌方的塔--------------------------------------------------------------------
				if #enemyheroes == 0 then
					cTemp = CountAlliesNearPoint(enemytowers[1]:GetAbsOrigin(), enemytowers[1]:GetAttackRange(), hero,
						false, false)
					...
					if chase then
						if AIHasStrongerHeroForce(allyheroes, enemyheroes) then
							value = lifeper
							cTemp = CountAlliesNearPoint(enemytowers[1]:GetAbsOrigin(), enemytowers[1]:GetAttackRange(),
								hero, false, false)
...
function AIOrderHoldInFountain(hero)
	...
	local pOrg = hero:GetAbsOrigin()
	local location = Vector(math.random(youxia_zuobiao.x - zuoshang_zuobiao.x) + zuoshang_zuobiao.x,
		math.random(youxia_zuobiao.y - zuoshang_zuobiao.y) + zuoshang_zuobiao.y, youxia_zuobiao.z)
	hero:MoveToPosition(location)
	hero.headWayPoint = 1
	if hero.tc > hero.tc_equip then
		AIGetItem(hero)
...
function AIGank(hero)
	...
	local method = -1
	--------------收集数据---------------
	local units = FindUnitsInRadius(hero:GetTeam(), pOrg, nil, 1300, DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_ALL,
		DOTA_UNIT_TARGET_FLAG_NONE, FIND_CLOSEST, false)
	if #units > 0 then
		for _, unit in pairs(units) do
			if IsValidUnit(unit) and unit:IsAlive() then
...
function AIFollowStop(hero, target)
	...
	Notifications:BottomToTeam(hero:GetTeamNumber(),
		{ text = hero:GetUnitName(), duration = 5, class = "NotificationLine", style = { color = "green" } })
	Notifications:BottomToTeam(hero:GetTeamNumber(),
		{ text = "DOTA_Chat_Follow_End", class = "NotificationLine", continue = true })
	if target then
		Notifications:BottomToTeam(hero:GetTeamNumber(),
			{ text = target:GetUnitName(), class = "NotificationLine", style = { color = "green" }, continue = true })
	end
end
...
function AIOrderTPSave(building, attacker)
	...
	if building:GetHealthPercent() < 80 then
		local units = FindUnitsInRadius(building:GetTeam(), pOrg, nil, 1200, DOTA_UNIT_TARGET_TEAM_BOTH,
