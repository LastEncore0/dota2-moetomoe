...
function ExorcismStart( event )
	...
	local need_spirits = ability:GetLevelSpecialValueFor( "spirits", ability:GetLevel() - 1 )
	local str = caster:GetStrength() - ability:GetLevelSpecialValueFor( "min", ability:GetLevel() - 1 )
	local spirits = math.floor(caster:GetStrength() / need_spirits)
	...
	for i=1,spirits do
		Timers:CreateTimer(i * delay_between_spirits, function()
			...
			ability:ApplyDataDrivenModifier(caster, caster, "modifier_die_river", {})
...
function ExorcismEnd( event )
	...
	for _,unit in pairs(targets) do		
		...
		caster:RemoveModifierByName("modifier_die_river")
		-- Removes a modifier.
	end
