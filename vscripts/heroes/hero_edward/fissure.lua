...
function CreateFissure(keys)
	...
	local stun_duration1 = ability:GetLevelSpecialValueFor("stun_duration", (ability:GetLevel() -1))
    ...
    local fissure_agility = ability:GetLevelSpecialValueFor("fissure_agility", (ability:GetLevel() -1))
	local damagemath = ability:GetLevelSpecialValueFor("damagemath", (ability:GetLevel() -1))
	local stun_duration = stun_duration1 * Strength
    local range = fissure_range + Agility * fissure_agility
	-- Position and direction variables
	local direction = caster:GetForwardVector()
	local startPos = caster:GetAbsOrigin() + direction * offset
	local endPos = caster:GetAbsOrigin() + direction * range
    local damage = ability:GetAbilityDamage() + damagemath * Intellect
    ...
    print("range =",range)
	print("stun_duration =",stun_duration)
	print("damage =",damage)
	end
...
function CheckPosition(keys)
	...
	--[[
	local caster_vector_distance = caster:GetAbsOrigin() - ability.startPos
    local caster_distance = (caster_vector_distance):Length2D()
			local caster_direction = (caster_vector_distance):Normalized()
	
			-- Get the caster's angle in relation to the front of the fissure
			local caster_angle_radian = math.atan2(caster_direction.y, caster_direction.x)
			
			-- Converts to degrees (0-360)
			local caster_angle_from_fissure = math.deg(caster_angle_radian) + 180
		
			-- Get's the direction of the fissure in the world
			local fissure_angle_radian1 = math.atan2(direction.y, direction.x)
			
			-- Converts to degrees (0-360)
			local fissure_angle1 = math.deg(fissure_angle_radian1) + 180
			
			-- Gets the distance from the front of the fissure to the point on the fissure perpendicular to the caster
			local perpen_distance1 = math.abs(math.sin(fissure_angle_radian1 - caster_angle_radian)) * caster_distance
	
			-- The angle the caster's facing in the world
			local caster_angle = caster:GetAnglesAsVector().y

	if (caster_angle_from_fissure - fissure_angle1) < 0 and perpen_distance1 <= width + 20 and ((caster_angle - fissure_angle1 < 0 and caster_angle - fissure_angle1 > -180) or (caster_angle - fissure_angle1 > 180)) then
				-- Removes the movespeed minimum
					ability:ApplyDataDrivenModifier(caster, caster, "modifier_alchemist_2",{})

			else
				-- Removes the slowing debuffs, so the unit can move freely

				caster:RemoveModifierByName("modifier_alchemist_2")
			end
	]]
