...
function setpfx(event)
	...
	local units = FindUnitsInRadius(caster:GetTeamNumber(), vector, nil, flying_vision, ability:GetAbilityTargetTeam(),
		ability:GetAbilityTargetType(), 0, FIND_CLOSEST, false)
	--print("setpfx unit")
...
function fireice(event)
	...
	if caster:HasTalent("emilia_talent_2") then
		ApplyDamage(
			{
				victim = target,
				attacker = caster,
...
function ApplyFFFModifier(keys)
	...
	local initial_units = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, caster.emt_range,
		DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, ability:GetAbilityTargetFlags(), 0,
		false)
...
function heal_percent(keys)
	local caster = keys.caster
	local target = keys.target
	local ability = keys.ability
	local percent = ability:GetLevelSpecialValueFor("percent", (ability:GetLevel() - 1))/100
	local heal = target:GetMaxHealth() * percent

	if caster:HasTalent("emilia_talent_1") then
		caster:Heal(heal,target)
		print("heal =",heal)
	end
	
end

function talent_ApplyModifier(keys)
	local caster = keys.caster
	local ability = keys.ability
	local max_range = ability:GetLevelSpecialValueFor("max_radius", (ability:GetLevel() - 1))
	local incrsing = ability:GetLevelSpecialValueFor("incrsing", (ability:GetLevel() - 1))
	...
	-- local duration = ability:GetLevelSpecialValueFor("duration", (ability:GetLevel() -1))
	local talent_time = ability:GetLevelSpecialValueFor("talent_slow_time", (ability:GetLevel() - 1))
	-- Renders the echoslam particle around the caster
	local Passive_range = caster:Script_GetAttackRange()

	local startPos = caster:GetAbsOrigin()

	if caster:HasTalent("emilia_talent_3") then
		if caster.fmpfx ~= nil then
			ParticleManager:DestroyParticle(caster.fmpfx, false)
		end
	
		local particleName1 = "particles/units/heroes/emiliya/emilia_cold_aura.vpcf"
		local particleName2 = "particles/units/heroes/emiliya/emilia_hot_aura.vpcf"
		if caster:HasModifier("modifier_temperature_mastery") then
			caster.fmpfx = ParticleManager:CreateParticle(particleName2, PATTACH_ABSORIGIN_FOLLOW, caster)
		else
			caster.fmpfx = ParticleManager:CreateParticle(particleName1, PATTACH_ABSORIGIN_FOLLOW, caster)
		end
		ParticleManager:SetParticleControl(caster.fmpfx, 1, Vector(Passive_range, Passive_range, Passive_range))

	
		local initial_units = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, Passive_range,
			DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, ability:GetAbilityTargetFlags(), 0,
			false)
		...
		for i, initial_unit in ipairs(initial_units) do
			if initial_unit ~= caster then
				if caster:HasModifier("modifier_temperature_mastery") then
					ability:ApplyDataDrivenModifier(caster, initial_unit, "modifier_burn", {})
					ApplyDamage({ victim = initial_unit, attacker = caster, ability = ability,
								damage = damage * 3, damage_type = ability:GetAbilityDamageType() })
				else
					if not initial_unit:HasModifier("modifier_frozen_talent") then
						ability:ApplyDataDrivenModifier(caster, initial_unit, "modifier_frozen_talent", { Duration = talent_time })
					end
				end
			end
		end
...
function removeFModifier(keys)
	...
	local initial_units = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, max_range,
		DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, ability:GetAbilityTargetFlags(), 0,
		false)
	-- ApplyDamage({victim = units, attacker = caster, damage = damage, damage_type = DAMAGE_TYPE_MAGICAL})
			...
			if caster:HasModifier("modifier_temperature_mastery") then
				if target:HasModifier(modifier1) then
					...
					if current_stack1 < 1 then
						...
						ApplyDamage({ victim = target, attacker = caster, ability = ability, damage = damage,
							damage_type = ability:GetAbilityDamageType() })
					else
						print("max", max)
						if current_stack1 < max then
							target:SetModifierStackCount(modifier1, caster, current_stack1 + 1)
							ApplyDamage({ victim = target, attacker = caster, ability = ability,
								damage = damage * current_stack1, damage_type = ability:GetAbilityDamageType() })
						else
							target:SetModifierStackCount(modifier1, caster, max)
							ApplyDamage({ victim = target, attacker = caster, ability = ability, damage = damage * max,
								damage_type = ability:GetAbilityDamageType() })
						end
					end
				else
