LinkLuaModifier("modifier_chronosphere_speed_lua", "heroes/hero_homura/modifiers/modifier_chronosphere_speed_lua.lua", LUA_MODIFIER_MOTION_NONE)
...
--[[Author: Pizzalol
    ...
    function LandMinesTracker( keys )
        ...
        local ability = keys.ability
        local ability_level = ability:GetLevel() - 1
        local casterAbsOrigin = target:GetAbsOrigin()
        local trigger_range = ability:GetLevelSpecialValueFor("vision_radius", ability_level) 
        ...
        if #units > 0 then
            Timers:CreateTimer(explode_delay, function()
                if target:IsAlive() then
                    ...
                    -- print("sound_explosion",sound_explosion)
	                EmitSoundOn(sound_explosion, target)
                    local boom_particle = "particles/units/heroes/hero_techies/techies_land_mine_explode.vpcf"
                    local particle_explosion_fx = ParticleManager:CreateParticle(boom_particle, PATTACH_WORLDORIGIN, target)
                    ...
                    target: RemoveModifierByName("modifier_land_mine_datadriven")
    ...
    if caster.abilityName == "landmine" then
        caster:SwapAbilities("houmura_gun", "landmine", true, false) 
    end
    if caster.abilityName == "houmura_gun" then
        caster:SwapAbilities("Flash_bomb", "houmura_gun", true, false)   
        local houmura_gun = caster:FindAbilityByName("houmura_gun")
        if houmura_gun:GetToggleState() == true then
            houmura_gun:ToggleAbility()
            end
            caster:RemoveModifierByName("modifier_houmura_gun")
        end
   if caster.abilityName == "Flash_bomb" and caster:HasTalent("homura_talent_1") then
    caster:SwapAbilities( "houmura_tank", "Flash_bomb",true, false) 
elseif caster.abilityName == "Flash_bomb" and caster:HasTalent("homura_talent_2") then
    caster:SwapAbilities( "houmura_fly", "Flash_bomb",true, false) 
elseif caster.abilityName == "Flash_bomb" and caster:HasTalent("homura_talent_3") then
    caster:SwapAbilities( "houmura_sub", "Flash_bomb",true, false) 
   elseif caster.abilityName == "Flash_bomb" then  
    caster:SwapAbilities( "homura_rpg", "Flash_bomb",true, false)  
    end
    if caster.abilityName == "houmura_tank" and caster:HasTalent("homura_talent_2") then
        caster:SwapAbilities("houmura_fly", "houmura_tank",true, false) 
    elseif caster.abilityName == "houmura_tank" and caster:HasTalent("homura_talent_3") then
        caster:SwapAbilities( "houmura_sub", "houmura_tank",true, false) 
    elseif caster.abilityName == "houmura_tank" then  
        caster:SwapAbilities( "homura_rpg", "houmura_tank",true, false)   
        end
        if caster.abilityName == "houmura_fly" and caster:HasTalent("homura_talent_3") then
            caster:SwapAbilities("houmura_sub","houmura_fly",true, false)  
        elseif caster.abilityName == "houmura_fly" then  
            caster:SwapAbilities( "homura_rpg", "houmura_fly",true, false) 
            end
            if caster.abilityName == "houmura_sub" then
                caster:SwapAbilities( "homura_rpg", "houmura_sub",true, false) 
            end
