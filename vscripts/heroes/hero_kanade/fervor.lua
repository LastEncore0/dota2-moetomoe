...
function RemoveDamage(keys)
	...
	if caster:HasTalent("kanade_talent_1") then
		local duration = reflect_duration + add_duration
		ability:ApplyDataDrivenModifier(caster, caster, "modifier_spiked_carapace_buff_datadriven",
			{ duration = duration })
	else
...
function BlinkStrike(keys)
	...
	local attacker_new = Vector(victim_position.x - 100 * math.cos(victim_angle_rad),
		victim_position.y - 100 * math.sin(victim_angle_rad), 0)
...
function CheckBackstab(params)
	...
	ParticleManager:SetParticleControlEnt(particle, 1, params.target, PATTACH_POINT_FOLLOW, "attach_hitloc",
		params.target:GetAbsOrigin(), true)
	...
	--end
	if caster:HasItemInInventory("item_echo_sabre") or caster:HasItemInInventory("item_harpoon") then
		if random < 0.5 then
...
function PrimalSplit(event)
	...
	if caster:HasTalent("kanade_talent_3") then
		for i = 1, 4 do
...
function EchoSlam(keys)
	...
	ParticleManager:SetParticleControl(particle1, 0,
		Vector(caster:GetAbsOrigin().x, caster:GetAbsOrigin().y, caster:GetAbsOrigin().z + caster:GetBoundingMaxs().z))
	ParticleManager:SetParticleControl(particle1, 1,
		Vector(echo_slam_damage_range, echo_slam_damage_range, caster:GetAbsOrigin().z + caster:GetBoundingMaxs().z))
	ParticleManager:SetParticleControl(particle1, 2,
		Vector(caster:GetAbsOrigin().x, caster:GetAbsOrigin().y, caster:GetAbsOrigin().z + caster:GetBoundingMaxs().z))
	...
	ParticleManager:SetParticleControl(particle2, 0,
		Vector(caster:GetAbsOrigin().x, caster:GetAbsOrigin().y, caster:GetAbsOrigin().z + caster:GetBoundingMaxs().z))
	ParticleManager:SetParticleControl(particle2, 1,
		Vector(echo_slam_damage_range, echo_slam_damage_range, caster:GetAbsOrigin().z + caster:GetBoundingMaxs().z))
	ParticleManager:SetParticleControl(particle2, 2,
		Vector(caster:GetAbsOrigin().x, caster:GetAbsOrigin().y, caster:GetAbsOrigin().z + caster:GetBoundingMaxs().z))

	-- Units to take the initial echo slam damage, and to send echo projectiles from
	local initial_units = FindUnitsInRadius(caster:GetTeamNumber(), caster:GetAbsOrigin(), nil, echo_slam_damage_range,
