...
function MeatHook( keys )
    ...
    caster.basespeed = 3 * ability:GetLevelSpecialValueFor("speed", (ability:GetLevel() - 1))
	caster.basedistance = (caster.point - caster_location):Length2D()
	caster.basedirection = (caster.point - caster_location):Normalized()
	caster.baseduration = caster.basedistance/caster.basespeed
	else
	caster.basespeed = ability:GetLevelSpecialValueFor("speed", (ability:GetLevel() - 1))
	caster.basedistance = (caster.point - caster_location):Length2D()
	caster.basedirection = (caster.point - caster_location):Normalized()
	caster.baseduration = caster.basedistance/caster.basespeed
    end
	...
	caster.hook_distance = distance
	caster.hook_speed = caster.basedistance * 1/30 -- 1/30 is how often the motion controller ticks
	caster.hook_direction = caster.basedirection
	caster.hook_traveled_distance = 0
...
function TimeWalkMotion( keys )
	...
	-- Move the caster while the distance traveled is less than the original distance upon cast
 if caster.wall_hit == true then
	if ability.hook_traveled_distance < ability.hook_distance then
		caster:SetAbsOrigin(caster:GetAbsOrigin() + ability.hook_direction * ability.hook_speed)
