...
function powershot_start_traverse( keys )
	...
	local caster = keys.caster
	local ability = keys.ability
	local startAttackSound = "Ability.static.loop"
	...
	Timers:CreateTimer( function()
			...
			local maxdamage = nil
					   if caster:HasTalent("misaka_talent_1") then
					   maxdamage = ability:GetLevelSpecialValueFor("damage", ability:GetLevel() - 1)
					   else
					   maxdamage = ability:GetAbilityDamage()
					   end
	if caster:HasItemInInventory("item_aether_lens") then
     damage = maxdamage * ability.powershot_damage_percent * 2
	else
	 damage = maxdamage * ability.powershot_damage_percent
	end
	print("damage =",damage)
			...
			for k, v in pairs( ability.powershot_units_array ) do
				...
				if ability.powershot_units_hit[ k ] == false and powershot_distance( v:GetAbsOrigin(), ability.powershot_currentPos ) <= ability.powershot_radius then
					-- Deal damage
					if caster:HasTalent("misaka_talent_1") then
					   ability.maxdamage = ability:GetLevelSpecialValueFor("damage", ability:GetLevel() - 1)
					   else
					   ability.maxdamage = ability:GetAbilityDamageType()
					   end
					local damageTable =
					{
						victim = v,
...
function removefiled( keys )
	local caster = keys.caster
	local ability = keys.ability
	local cooldown1 = ability:GetCooldown(ability:GetLevel() - 1)
	local cooldown2 = ability:GetLevelSpecialValueFor("cold", ability:GetLevel() -1)
	-- Return a handle to the modifier of the given name if found, else nil (string Name )
	if caster:HasTalent("misaka_talent_2") then
	ability:StartCooldown(cooldown2)
	ability:ApplyDataDrivenModifier(caster, caster, modifier_zhiding1, { Duration = cooldown2 } )
	else
	ability:StartCooldown(cooldown1)
	ability:ApplyDataDrivenModifier(caster, caster, modifier_zhiding1, { Duration = cooldown1 } )
    end
end

function readycd( keys )
	local caster = keys.caster
	local ability = keys.ability

	if ability:IsCooldownReady() then
	caster:RemoveModifierByName("modifier_nodiancichang")
	ability:ApplyDataDrivenModifier(caster, caster, "modifier_diancichang", { Duration = cooldown1 } )
    end
end
