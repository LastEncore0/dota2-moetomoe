...
function invoker_chaos_meteor_datadriven_on_spell_start(keys)
	...
	local chaos_meteor_fly_particle_effect = ParticleManager:CreateParticle("particles/units/heroes/reimu/invoker_chaos_meteor_fly.vpcf", PATTACH_ABSORIGIN, keys.caster)
...
--[[ 	
	...
	--if exort_ability ~= nil then
		local exort_level = keys.ability:GetLevel()
	...
	--if wex_ability ~= nil then
		travel_distance = keys.ability:GetLevelSpecialValueFor("travel_distance", keys.ability:GetLevel() - 1)
	--end
	...
	Timers:CreateTimer({
		...
		callback = function()
			...
			chaos_meteor_dummy_unit:AddAbility("YinYang_Orbs")
			local chaos_meteor_unit_ability = chaos_meteor_dummy_unit:FindAbilityByName("YinYang_Orbs")
			if chaos_meteor_unit_ability ~= nil then
				--chaos_meteor_unit_ability:SetLevel(1)
				keys.ability:ApplyDataDrivenModifier(chaos_meteor_dummy_unit, chaos_meteor_dummy_unit, "modifier_invoker_chaos_meteor_datadriven_unit_ability", {duration = -1})
			end
			...
			{
				EffectName = "particles/units/heroes/reimu/invoker_chaos_meteor.vpcf",
...
function stopSound( event )
	StopSoundEvent("Hero_Invoker.ChaosMeteor.Cast",event.caster)
	StopSoundEvent("Hero_Invoker.ChaosMeteor.Loop",event.caster)
	StopSoundEvent("Hero_Invoker.ChaosMeteor.Damage",event.caster)
	print("stopSound")
end