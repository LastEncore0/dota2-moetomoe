...
function CastSpirits( event )
	...
	ability.reimu_startTime		= GameRules:GetGameTime()
	ability.reimu_numSpirits		= 0		-- Use this rather than "#spirits_spiritsSpawned"
	ability.reimu_spiritsSpawned	= {}
...
function ThinkSpirits( event )
	...
	local elapsedTime	= GameRules:GetGameTime() - ability.reimu_startTime
	...
	if ability.reimu_numSpirits < idealNumSpiritsSpawned then
        ...
        print("ability.reimu_numSpirits =",ability.reimu_numSpirits)
		-- Update the state
		local spiritIndex = ability.reimu_numSpirits + 1
		newSpirit.spirit_index = spiritIndex
		ability.reimu_numSpirits = spiritIndex
		ability.reimu_spiritsSpawned[spiritIndex] = newSpirit
	...
	for k,v in pairs( ability.reimu_spiritsSpawned ) do
	...
	if ability.reimu_numSpirits == numSpiritsMax and numSpiritsAlive == 0 then
...
function OnDestroySpirit( event )
	...
	local intmath = ability:GetLevelSpecialValueFor("intmath", (ability:GetLevel() -1))
	local intdamage = damage + intmath * int
...
function OnDestroydamage( event )
    local caster	= event.caster
	local ability	= event.ability
	local int = caster:GetIntellect()
	local damage = ability:GetLevelSpecialValueFor("damage", (ability:GetLevel() -1))
    ...
    for i,unit in ipairs(units) do
			...
			ApplyDamage({victim = unit, attacker = caster, damage = intdamage, damage_type = ability:GetAbilityDamageType()})
		end
...
function ExplodeSpirit( event )
	...
	if not spirit.spirit_isExploded then
		...
		ability.reimu_spiritsSpawned[spirit.spirit_index] = nil
