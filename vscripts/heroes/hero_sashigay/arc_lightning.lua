...
function stream(keys)
	...
	if caster:HasTalent("sasuke_talent_1") then
	caster.caster_attack = caster:GetAttackCapability()
    caster.tbs = true
	caster:SetRangedProjectileName(projectile_model)
	caster:SetAttackCapability(DOTA_UNIT_CAP_RANGED_ATTACK)
	ability:ApplyDataDrivenModifier(caster, caster, "modifier_thousand_bird_2", {})
	end
end
...
function NewInstance(keys)
	...
	if caster:HasTalent("sasuke_talent_1") or caster.tbs == true then
    caster.tbs = false
	caster:SetAttackCapability(caster.caster_attack)
	caster:RemoveModifierByName("modifier_thousand_bird_2")
	end
...
function huanshu(keys)
	local caster = keys.caster
	local ability = keys.ability
	local target = keys.target
	
	if caster:GetIntellect() >= target:GetIntellect() then
	ability:ApplyDataDrivenModifier(caster, target, "modifier_sashigay_huanshu", {})
	else
	ability:ApplyDataDrivenModifier(caster, target, "modifier_sashigay_huanshu_2", {})
	end
end
...
function sleep(keys)
	...
	ability:ApplyDataDrivenModifier(caster, target, "modifier_sashigay_huanshu_3", {})
	end
end
