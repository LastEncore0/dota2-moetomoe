...
function GetLastSpell(keys)
	...
	for i = 0, 15 do
		...
		print("current_ability =", current_ability:GetAbilityName())
	...
	if target.last_ability ~= nil then
		...
		print("new_ability_name =", new_ability_name)
		local ban_abilityNames = {"star_burst_stream", "xielunyan", "double_gunandknife"}
		if notInTable(new_ability_name, ban_abilityNames) then
			
			
			if caster:HasModifier("modifier_xielunyan_2") == false then
				if caster:HasModifier("modifier_spell_steal_datadriven") == false then
					...
					ability:ApplyDataDrivenModifier(caster, caster, "modifier_xielunyan_2", {})
				...
				elseif new_ability_name ~= ability.current_steal:GetAbilityName() then
					...
					caster:FindAbilityByName(new_ability_name):SetLevel(ability_level)
					if ability.current_steal:GetAbilityName() ~= ability:GetAbilityName() then
						caster:RemoveAbility(ability.current_steal:GetAbilityName())
					end
					ability:ApplyDataDrivenModifier(caster, caster, "modifier_xielunyan_2", {})
				end
				if ability.new_steal ~= nil then
					ability.current_steal = ability.new_steal
					ability.current_level = new_ability_level
				end
				print("ability.current_steal =", ability.current_steal)
				print("ability.new_steal =", ability.new_steal)
				if caster:HasTalent("sasuke_talent_2") then
...
function SpellSteal(keys)
	...
	if new_ability_name ~= "star_burst_stream" then
		...
		elseif new_ability_name ~= ability.current_steal:GetAbilityName() then
			caster:AddAbility(new_ability_name)
			caster:SwapAbilities(ability.current_steal:GetAbilityName(), ability_name, false, true)
			caster:FindAbilityByName(new_ability_name):SetLevel(ability_level)
		end
...
function RemoveSpell(keys)
	...
	if ability.current_steal ~= nil then
		...
		if new_ability_name ~= ability:GetAbilityName() then
...
function FixLevels(keys)
	...
	if ability.current_steal ~= nil then
		...
		local correct_ability_level = ability.current_level
