...
function CreateFissure(keys)
	...
	local damge = ability:GetLevelSpecialValueFor("damge", (ability:GetLevel() -1))
	
	-- Position and direction variables
	local direction = caster:GetForwardVector()
	local startPos = caster:GetAbsOrigin() + direction * offset  + 100
	...
	for j,unit in ipairs(units) do
		...
		unit:AddNewModifier(caster, ability, "modifier_covearth", {Duration = stun_duration})
		-- Applies the damage to the target
		ApplyDamage({victim = unit, attacker = caster, damage = damge, damage_type = ability:GetAbilityDamageType()})
	end
