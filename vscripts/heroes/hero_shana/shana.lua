LinkLuaModifier("modifier_movespeed_cap_low", "libraries/modifiers/modifier_movespeed_cap_low.lua",
...
function CreateFissure(keys)
	...
	local units = FindUnitsInLine(caster:GetTeam(), startPos, endPos, nil, width, DOTA_UNIT_TARGET_TEAM_BOTH,
	...
	units = FindUnitsInLine(caster:GetTeam(), startPos, endPos, nil, fissure_radius, DOTA_UNIT_TARGET_TEAM_ENEMY,
		DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0)
...
function CheckPosition(keys)
	...
	local units = FindUnitsInLine(caster:GetTeam(), startPos, endPos, nil, width + 20, DOTA_UNIT_TARGET_TEAM_BOTH,
		DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0)
...
function modifier_flamefly_mana_cost(keys)
	...
	if not caster:HasTalent("shana_talent_1") then
...
function neverdie(keys)
	...
	local modfier = keys.modfier
	-- local modifier_flamewing = keys.modifier_flamewing
	if caster:HasTalent("shana_talent_3") then
...
function Alastor(keys)
	...
	-- local modifier_flamewing = keys.modifier_flamewing
	if caster:HasTalent("shana_talent_2") then
...
function AssassinateParticleStart(keys)
	...
	target.assassinate_crosshair_pfx = ParticleManager:CreateParticleForTeam(particle_debuff, PATTACH_OVERHEAD_FOLLOW,
		target, caster:GetTeam())
...
function AssassinateHit(keys)
	...
	ParticleManager:SetParticleControlEnt(pfx, 0, caster, PATTACH_POINT_FOLLOW, "attach_hitloc", caster:GetAbsOrigin(),
		true)
end
...
function CastSunRay(event)
	...
	if caster:HasItemInInventory("item_radiance") then
		...
		ability:ApplyDataDrivenModifier(caster, caster, "modifier_Conviction_check", {})
		ability_handle:SetLevel(this_abilityLevel)
		-- else
...
function EndSunRay(event)
	...
	if caster:HasModifier("modifier_Conviction_check") then
		print("has",modifier_cov_check)
		
		caster:RemoveModifierByName("modifier_Conviction_check")
	else
		caster:SwapAbilities(Conviction, stopsun, true, false)
		print("not has",modifier_cov_check)
	end
